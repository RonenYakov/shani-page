...
const BrandsVideoCarousel = () => {
  ...
  const brandMap = useMemo(() => (
    import.meta.glob('@/assets/brands/*.{mp4,webm}', { query: '?url', import: 'default', eager: true }) as Record<string, string>
  ), []);
  ...
  // 5 best brand videos
  const brands = [
    { file: 'mahlevet evri.mp4',        poster: '/posters/brands/mahlevet evri.webp',       title: 'תדמית מסעדה',    titleEn: 'Restaurant Brand Film', category: 'קמפיין',    categoryEn: 'Campaign' },
    { file: 'streets.mp4',              poster: '/posters/brands/streets.webp',              title: 'שאלון רחוב',     titleEn: 'Street Interview',      category: 'סושיאל',   categoryEn: 'Social' },
    { file: 'ugc-B.mp4',                poster: '/posters/brands/ugc-B.webp',               title: 'UGC',            titleEn: 'UGC Compilation',        category: 'תוכן',     categoryEn: 'Content' },
    { file: 'סרטון תדמית מסעדה-B.mp4', poster: '/posters/brands/סרטון תדמית מסעדה-B.webp', title: 'תדמית מסעדה',   titleEn: 'Restaurant Brand Film', category: 'קמפיין',    categoryEn: 'Campaign' },
    { file: 'agalt-cafe.mp4',           poster: '/posters/brands/agalt-cafe.webp',           title: 'עגלת קפה',      titleEn: 'Coffee Cart',           category: 'סושיאל',   categoryEn: 'Social' },
        ...
        <div ref={containerRef} dir="ltr" className="relative overflow-x-auto overflow-y-hidden scrollbar-hide pb-8" style={{ scrollSnapType: 'x mandatory', scrollBehavior: 'smooth' }}>
          ...
          <div className="flex gap-6 md:gap-8 lg:gap-10 min-w-max">
            {brands.map((item, index) => (
              ...
              >
                {/* Media area — poster by default, video only when hovered */}
...
const ScrollHintArrow = ({ containerRef, direction = 'right', topPx }: { containerRef: React.RefObject<HTMLDivElement>; direction?: 'left' | 'right'; topPx?: number | null }) => {
  ...
  const onClick = () => {
    const el = containerRef.current;
    if (!el) return;
    el.scrollBy({ left: el.clientWidth * 0.8 * (direction === 'right' ? 1 : -1), behavior: 'smooth' });
  };

  return (
    <motion.button aria-label={direction === 'right' ? 'Scroll right' : 'Scroll left'} onClick={onClick}
